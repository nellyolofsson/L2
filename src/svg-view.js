...
/**
 * Svgview class for performing svg image on electricityprice data.
 */
export class Svgview {
  #PriceStatisticsGenerator
  /**
   * Constructor for SvgView.
   *
   */
  constructor () {
    this.#PriceStatisticsGenerator = new PriceStatisticsGenerator()
  }

  /**
   * Generates and updates price data, optionally historical.
   *
   * @param {boolean} isHistorical - Indicates whether historical data should be generated.
   * @returns {string} The generated HTML containing price data.
   */
  async generateAndUpdatePriceData (isHistorical = false) {
    try {
      const regionNames = {
        0: 'Norra Sverige',
      ...
      let priceStatistics
      if (isHistorical) {
        priceStatistics = await this.#PriceStatisticsGenerator.generateHistoricalPriceStatistics()
      } else {
        priceStatistics = await this.#PriceStatisticsGenerator.generateTodayPriceStatistics()
      }
  ...
  /**
   * Generates an SVG based on the provided data.
   *
   * @param {string} regionCode - The code of the region.
   * @param {string} regionName - The name of the region.
   * @param {object} stats - An object containing price statistics.
   * @returns {string} The SVG template as a string.
   */
  ...
  /**
   * Generate the SVG and image string.
   *
   * @param {boolean} isHistorical - Indicates whether historical data should be used.
   * @returns {string} The SVG and image string.
   */
  getImageDiagramString (isHistorical = false) {
    return this.generateAndUpdatePriceData(isHistorical)
  }
}
